...
#%%
loom_15_sec = freeze_loom.groupby(np.arange(len(freeze_loom))//15).mean()
...
loom_15_sec['min']=  (loom_15_sec['15sec']*15/60) - 10
...
shock_15_sec['min']=  (shock_15_sec['15sec']*15/60) - 10

#%%
ax = sns.lineplot(x='min', y='percent', data =shock_15_sec, color = 'crimson').set_title('Tone-Shock conditioning\n', fontsize=20)#T-Shock
...
ax.axvspan(0,30, alpha=0.3, color='lightgray')
ax.set_ylim([-0, 100])
ax.set_xlim([-10,28])
ax.set_ylabel('Freezing %', fontsize=20)
ax.set_xlabel('Minutes', fontsize=20)
...
ax = plt.fill_between(shock_15_sec['min'],shock_15_sec['lower_bound'] , shock_15_sec['upper_bound'], color ='crimson',alpha=.3)
ax = sns.lineplot(x=[10,10], y =[0,1.0], color = 'lightgray')
ax.axvspan(0,30, alpha=0.2, color='gray')
...
plt.savefig('cond_lineplot.jpeg',bbox_inches='tight', dpi=1800)
